...
if [[ -d "$(pwd)"/auto_spy_data/autospy  ]]; then
	...
	docker rm auto_spy
	echo "autospy配置文件已存在"
	docker run -dit --restart=always --network=host --name=auto_spy --log-opt max-size=100m --log-opt max-file=3 -v "$(pwd)"/auto_spy_data/autospy:/autospy --hostname=auto_spy xieshang1111/auto_spy:$arch
else
	mkdir auto_spy_data
